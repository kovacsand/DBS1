...
SELECT DISTINCT duration
FROM phone_calls a
WHERE 3 >= (SELECT COUNT(DISTINCT duration)
            FROM phone_calls b
            WHERE a.duration <= b.duration)
ORDER BY a.duration DESC;

SELECT caller, MAX(duration) as _max
FROM phone_calls
GROUP BY caller
ORDER by _max;
...
SELECT *
FROM people JOIN passengers p on people.passport_number = p.passport_number JOIN flights f on p.flight_id = f.id
WHERE f.month = 7 AND f.month != 7;

SELECT passport_number, count(*)
FROM passengers p, flights f
WHERE p.flight_id = f.id
    AND f.day = 28
    AND p.passport_number IN (SELECT p1.passport_number FROM flights f1, passengers p1 WHERE f1.id = p1.flight_id AND f1.day = 27)
GROUP BY passport_number
HAVING count(*) > 1
ORDER BY passport_number;

SELECT passport_number FROM passengers p, flights f WHERE p.flight_id = f.id AND f.day = 27 ORDER BY passport_number;

SELECT *
FROM people JOIN passengers p on people.passport_number = p.passport_number JOIN flights f on p.flight_id = f.id
WHERE people.name = 'Melissa';